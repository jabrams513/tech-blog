...
// Importing functions to seed users, posts, and comments
const seedUsers = require('./userData');
const seedPosts = require('./postData');
const seedComments = require('./commentData');

// Function to seed the entire database with users, posts, and comments
const seedDB = async () => {
  try {
    // Synchronize the database and force it to drop and recreate tables
    await sequelize.sync({ force: true });

    // Call functions to seed users, posts, and comments
    await seedUsers();
    await seedPosts();
    await seedComments();
